...
import torch
from models.modernn import MoDeRNN_Cell
import sys
sys.path.append("..")
from config import cfg


class MS_MoDeRNN(nn.Module):
    def __init__(self, input_channel, output_channel, b_h_w, kernel_size, stride, padding):
        super().__init__()
        self.n_layers = cfg.LSTM_layers
        B, H, W = b_h_w
        lstm = [MoDeRNN_Cell(input_channel, output_channel, [B, H, W], kernel_size, stride, padding),
                MoDeRNN_Cell(input_channel, output_channel, [B, H // 2, W // 2], kernel_size, stride, padding),
                MoDeRNN_Cell(input_channel, output_channel, [B, H // 4, W // 4], kernel_size, stride, padding),
                MoDeRNN_Cell(input_channel, output_channel, [B, H // 4, W // 4], kernel_size, stride, padding),
                MoDeRNN_Cell(input_channel, output_channel, [B, H // 2, W // 2], kernel_size, stride, padding),
                MoDeRNN_Cell(input_channel, output_channel, [B, H, W], kernel_size, stride, padding)]
