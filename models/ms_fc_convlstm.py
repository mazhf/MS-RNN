...
class Multi_Scale_Fully_Connected_ConvLSTM(nn.Module):
    def __init__(self, input_channel, output_channel, b_h_w, kernel_size, stride, padding):
        ...
        self.ups_1 = [nn.Upsample(scale_factor=2, mode='bilinear')]
        self.ups_2 = [nn.Upsample(scale_factor=2, mode='bilinear'), nn.Upsample(scale_factor=4, mode='bilinear')]
        ...
        self.concat_2 = nn.Conv2d(input_channel * 2, output_channel, 1, 1, 0)
        self.concat_3 = nn.Conv2d(input_channel * 3, output_channel, 1, 1, 0)
        self.concat_4 = nn.Conv2d(input_channel * 4, output_channel, 1, 1, 0)
        self.concat_5 = nn.Conv2d(input_channel * 5, output_channel, 1, 1, 0)
    ...
    def forward(self, x, m, layer_hiddens, embed, fc):
        ...
        for l in range(self.n_layers):
            ...
            if l == 0:
                ...
                x = torch.cat([out_1_down, out_0_down_down], dim=1)
                x = self.concat_2(x)
            elif l == 2:
                out_2_up = self.ups_2[0](x)
                out_2_up_up = self.ups_2[1](x)
                x = torch.cat([out_1_down, out_0_down_down, x], dim=1)
                x = self.concat_3(x)
            elif l == 3:
                out_3_up = self.ups_3[0](x)
                out_3_up_up = self.ups_3[1](x)
                x = torch.cat([out_0_down, raw_out[1], out_2_up, out_3_up], dim=1)
                x = self.concat_4(x)
            elif l == 4:
                out_4_up = self.ups_4[0](x)
                x = torch.cat([raw_out[0], out_1_up, out_2_up_up, out_3_up_up, out_4_up], dim=1)
                x = self.concat_5(x)
