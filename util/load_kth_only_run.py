...
def load_file(data_root, data_type, seq_len, T):
    if data_type == 'test':
        file_path = os.path.join(os.path.join(os.path.dirname(data_root), 'kth_txt'), 'test_data_list_only_run.txt')
    else:
...
class KTH_Only(object):
    def __init__(self, train, seq_len=20, T=20, image_size=128):
