...
def read_moving_data(path, mode):
    # mnist数据集本身是随机的.直接用切片划分np数组,按照7:1:2划分数据集
    data_all = np.load(path + '.npy')
    s, b, h, w = data_all.shape
    if mode == 'train':
        data = data_all[:, :b * 7 // 10, :, :]
    elif mode == 'valid':
        data = data_all[:, b * 7 // 10:b * 8 // 10, :, :]
    elif mode == 'test':
        data = data_all[:, b * 8 // 10:, :, :]
    return data
...
class Data(Dataset):
    def __init__(self, mode=''):
        ...
        # self.IN_LEN = cfg.BENCHMARK.IN_LEN
        # self.OUT_LEN = cfg.BENCHMARK.OUT_LEN
