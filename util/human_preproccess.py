...
def get_data():
    train_input_handle, test_input_handle = data_provider(cfg.GLOBAL.DATASET_PATH, batch_size=1,
                                                          img_width=cfg.width,
                                                          seq_length=cfg.in_len + cfg.out_len)
    train_data = []
    while not train_input_handle.no_batch_left():
        train_batch = train_input_handle.get_batch()  # (1, 8, 100, 100, 3)
        train_batch = train_batch.squeeze(axis=0).transpose((0, 3, 1, 2))
        train_data.append(train_batch)
        train_input_handle.next()
    test_data = []
    while not test_input_handle.no_batch_left():
        test_batch = test_input_handle.get_batch()  # (1, 8, 100, 100, 3)
        test_batch = test_batch.squeeze(axis=0).transpose((0, 3, 1, 2))
        test_data.append(test_batch)
        test_input_handle.next()
    return np.array(train_data), np.array(test_data)


def data_provider(data_paths, batch_size, img_width, seq_length):
    input_param = {'paths': data_paths,
...
class DataProcess:
    ...
    def load(self, paths, mode='train'):
        data_dir = paths
        intervel = 2
        frames_np = []
        scenarios = ['Walking']
        ...
        for filename in filenames:
            ...
            frames_np.append(np.array(image))
    ...
    def get_train_input_handle(self):
        train_data, train_indices = self.load(self.paths, mode='train')
        return InputHandle(train_data, train_indices, self.input_param)

    def get_test_input_handle(self):
        test_data, test_indices = self.load(self.paths, mode='test')
...
class InputHandle:
    ...
    def get_batch(self):
        if self.no_batch_left():
            logger.error(
                "There is no batch left in " + self.name + ". Consider to user iterators.begin() to rescan from the "
                                                           "beginning of the iterators")
            return None
